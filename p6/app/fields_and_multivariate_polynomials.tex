\chapter{Fields and Polynomials}
...
\end{definition}
\begin{remark}
  We will often use the notation $R[\underline{X}_{n}]$, instead of $R[X_{1}, \ldots, X_{n}]$.
\end{remark}
Alternatively if $n = 1, 2$ or $3$ we may write $R[X]$, $R[X, Y]$ or $R[X, Y, Z]$ instead of $R[\underline{X}_{n}]$.
...
\begin{definition}\label{def:homogenous_poly}
  Let $h \in R[\underline{X}_{n}]$. Suppose that there exists $d, k \in \N$, as well as $g_1, \ldots g_{n} \in R[\underline{X}_{n}]$ all monomials of degree $d$, and $a_{1}, \ldots, a_{n} \in R$ such that $h = \sum^{n}_{i = 1} a_{i} g_{i}$. Then $h$ is called \textbf{homogenous} and we define $\deg(h) := d$.
\end{definition}
Finally we are able to define the degree of an arbitrary multivariate polynomial.
\begin{definition}\label{def:degree}
  Let $f \in R[\underline{X}_{n}]$ then there exists some $d \in \N$ and $h_{1}, \ldots, h_{d} \in R[\underline{X}_{n}]$, all homogenous, with $\deg(h_{i}) = i$, such that $f = \sum^{d}_{i = 1} h_{i}$.
  Then we define the \textbf{degree} of $f$ to be $\deg(f) = d$.
\end{definition}
\begin{remark}\label{rem:deg_0}
  Since the $0 \in R[\underline{X}_{n}]$, can be written as the empty sum of homogenous polynomials, we will be using the convention that $\deg(0) = 0$.
\end{remark}
It is worth noting that if $f$ is a monomial or a homogenous polynomial, the degree defined in definition \ref{def:degree} agrees with the degree in \ref{def:monomial} or \ref{def:homogenous_poly} respectively.

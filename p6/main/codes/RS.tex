...
\begin{example}[Reed-Solomon Codes]
  ...
  As $1, X, \ldots, X^{k - 1}$, forms a basis of $L_{k}$, we see that $\C$ has dimension $k$, further more that
  \begin{equation*}
    G = \begin{bmatrix}
          1 & 1 & \cdots & 1\\
  ...
  In the special case $w = d$ where $d$ is the minimum distance of $\mathcal{C}$, we see that $n - d \leq k - 1$ rearranging we see that $n - k \leq d - 1$, combining this with the Singleton bound (Corollary \ref{cor:singleton_bound}) we get that $n - k = d - 1$, and thus $\mathcal{C}$ is a MDS code, with minimum distance $n - k + 1$. This is of cause a nice property to have, however these codes aren't the solution to all of our problems as we require $n \leq q$, so their length is limited.
\end{example}
We will later show that Reed-Solomon codes are a special case of our main object of study, namely algebraic geometry codes.

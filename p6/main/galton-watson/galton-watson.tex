...
Et eksempel på en stokastisk proces er forgreningsprocessen, som modellere et "et-kønnet" stamtræ, hvilket vil sige, at det kun er den ene forældre, som er med i stamtræet. 
...
\begin{prop} \label{prop:prop8.9}%8.9
    Lad en forgreningsproces have en forventede værdi $\mu$ af afkomsfordelingen. Hvis $\mu\leq1$, så er $P(\Omega)$=1. Ellers er $P(\Omega)<1$. 
\end{prop}
\begin{proof}
    Fra proposition \ref{prop 3.37} gælder det, at $\mu=G'(1)$. Hvis $p_X(0)=0$, så er $\mu > 1$ og $P(\Omega)=0$. Bemærk at beviset for specialtilfældet, hvor $G'(s)\equiv 1$ er undladt.
    \\
    \quad Antag nu at $p_X(0)>0$. Fra korollar \ref{problem133} vides det, at pgf'en $G(s)$ er konveks og voksende. Yderligere så er pgf'en stigende fra $G(0)=p_X(0)>0$ til $G(1)=1$. Hvis $\mu>1$, så må $G(s)$ krydse linjen $y=s$, samt er den første skæringen sandsynligheden, for at slægten uddør, hvilket er skarpt mindre end $1$ jævnfør proposition \ref{prop:prop8.7}.
